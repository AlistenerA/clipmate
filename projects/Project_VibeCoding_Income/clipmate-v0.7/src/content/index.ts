...
function collectTutorialVideoLinks(doc: Document, pageUrl: string): ClipVideoLinkInput[] {
  ...
  const videoHostPattern = /(?:youtube\.com|youtu\.be|vimeo\.com|bilibili\.com|youku\.com|qq\.com\/x\/cover)/i
  ...
  doc.querySelectorAll('video').forEach((node) => {
    add(node.getAttribute('src'), 'video', node.getAttribute('title') || node.getAttribute('aria-label'))
    node.querySelectorAll('source[src]').forEach((source) => {
      add(source.getAttribute('src'), 'source', node.getAttribute('title') || node.getAttribute('aria-label'))
    })
  })
