# PERMISSION_JUSTIFICATION.md — ClipMate v0.7 权限使用说明

> 上架 Edge Add-ons / Chrome Web Store 时提交的权限使用说明。
> 本说明对应 `manifest.config.ts` 中的实际权限配置。**v0.7 无权限变更。**
...
>
> **v0.7 权限变更**：无。教程结构解析、视频链接元数据和 Notion 原生 block 映射均在现有权限范围内实现；不会下载视频、抓取字幕或访问视频站 API。
...
| 权限 | v0.1 | v0.2 | v0.3 | v0.4 | v0.5-v0.7 | 说明 |
|------|:---:|:---:|:---:|:---:|:---:|------|
| `activeTab` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
| `storage` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
...
**v0.7 新增功能不依赖任何新增权限**。教程结构解析在当前页面 DOM 内完成；视频仅采集 HTTP(S) 链接元数据，不访问视频站 API、不下载内容、不读取 cookie。不调用 Notion File Upload API，不新增 FileReader/canvas/toDataURL/createObjectURL。
