# PRIVACY.md — ClipMate v0.7 Privacy Policy
...
- **Local Clipping History and Draft**: Stored locally in your browser. Entries may include page title, URL, tags, previews, Markdown, word count, target snapshot, save status, site metadata, lightweight image metadata, clip mode, and the current tutorial document structure. This data is used only by the extension and is never sent to a ClipMate-operated server.
...
- External image URLs and embedded video page/player URLs present in the clipped tutorial, when those elements are part of the content being saved
