# PRIVACY.md — ClipMate v0.9 Privacy Policy
...
- **Local Clipping History and Draft**: Stored locally in your browser. The current draft may include page title, URL, tags, notes, Markdown, tutorial structure, page-aware mode recommendation, and the external URLs of images you manually selected. History entries keep the existing lightweight image metadata unless those URLs are already present in saved Markdown. This data is used only by the extension and is never sent to a ClipMate-operated server.
...
- The **extracted body text and Markdown structure** of the webpage
- External image URLs that were extracted from the page or manually selected by the user, plus embedded video page/player URLs present in the clipped tutorial
...
### 8. v0.7 Tutorial Mode Privacy Additions

- Tutorial structure is derived locally from the current page and stored only as part of the local draft.
- Video support stores link metadata only. ClipMate does not download video, fetch subtitles, read cookies, or contact video providers directly.
- Video links are sent only to Notion when the user explicitly saves the clip, where they are represented as bookmark blocks.
- Tutorial mode adds no new permissions, analytics, telemetry, or external ClipMate service.

### 9. v0.8 Asset Picker Privacy Additions

- Asset Picker starts only after the user clicks “选择页面图片”. It reads visible image element attributes from the current page and temporarily adds a Shadow DOM selection overlay.
- Picker session state stays in the current tab's content-script memory. It is removed after the Popup consumes the result or the page is closed/navigated.
- Selected image URLs are stored only as part of the current local draft and are sent to Notion only when the user explicitly saves that draft.
- ClipMate does not download image bytes, contact image providers directly, read cookies, or create a site-level media cache.

### 10. v0.9 Page Awareness Privacy Additions

- Page type detection runs locally against the current document and does not call a remote model or analytics service.
- The recommendation stores only a page type, numeric confidence, selection-present boolean, mode identifiers, and a fixed extension-authored reason.
- Raw detector signals, page DOM, body text, selection text, URL, and detector debug reasons are not copied into the PageAwareness field.
- Page awareness adds no new permissions, network destinations, cookies, telemetry, or cloud storage.
