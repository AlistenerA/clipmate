# PERMISSION_JUSTIFICATION.md — ClipMate v0.9 权限使用说明

> 上架 Edge Add-ons / Chrome Web Store 时提交的权限使用说明。
> 本说明对应 `manifest.config.ts` 中的实际权限配置。**v0.9 无权限变更。**
...
>
> **v0.9 权限变更**：无。页面感知复用现有 content script 与本地 Page Type Detector；不会读取 cookie、访问第三方私有 API 或发送检测信号。
...
- **Page Awareness（v0.9 新增）**：本地计算页面结构类型与模式推荐，只向 Popup 返回脱敏的分类结果
...
| 权限 | v0.1 | v0.2 | v0.3 | v0.4 | v0.5-v0.9 | 说明 |
|------|:---:|:---:|:---:|:---:|:---:|------|
| `activeTab` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
| `storage` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
| `host_permissions: api.notion.com` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
| `content_scripts all_urls` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变（v0.9 只增加本地结构分类）|

**v0.9 新增功能不依赖任何新增权限**。页面感知只读取当前 DOM 的结构统计，返回固定字段，不发起网络请求、不读取 cookie、不保存原始 detector signals。
