...
import { cleanSourceText, extractFirstTopicOrSentence, isContainerCommentOnly } from './commentContextCleaners'
...
export function isGenericPlatformTitle(title: string, siteName?: string): boolean {
  ...
  if (siteName) {
    ...
    if (lowerTitle === lowerSite || lowerTitle === `${lowerSite}.com` || lowerTitle === `www.${lowerSite}.com`) {
      return true
    }
  }
  return false
...
function isCommentAreaElement(el: Element): boolean {
  ...
  while (current) {
    ...
    if (className.includes('comment') || className.includes('reply') || className.includes('avatar') || id.includes('comment') || id.includes('reply') || id.includes('avatar')) {
      return true
    }
...
function isAvatarImage(img: HTMLImageElement): boolean {
  ...
  const classStr = (img.className && typeof img.className === 'string') ? img.className.toLowerCase() : ''
...
function isWeiboBadgeOrEmojiImage(img: HTMLImageElement): boolean {
  const alt = (img.alt || '').toLowerCase()
  const classStr = (img.className && typeof img.className === 'string') ? img.className.toLowerCase() : ''
  const src = (img.src || '').toLowerCase()
  if (alt.includes('emoji') || alt.includes('表情') || alt.includes('emot')) return true
  if (/^\[.+\]$/.test(alt.trim()) && alt.length < 20) return true
  if (classStr.includes('emoji') || classStr.includes('emotion') || classStr.includes('face') || classStr.includes('badge') || classStr.includes('svip') || classStr.includes('member') || classStr.includes('level'))
    return true
  ...
  if (/\/face\//i.test(src)) return true
  ...
  if (src.includes('h5.sinaimg.cn/upload') && (src.includes('svip') || src.includes('member'))) return true
...
export function extractSourceTitle(
  ...
  if (sourceContainer) {
    ...
    if (heading) {
      ...
      if (headingText && headingText.length > 0 && headingText.length < 200) {
        return cleanSourceText(headingText, 200) || headingText.trim()
      }
    }
    const sourceText = extractSourceText(sourceContainer, 120)
    const cleanedText = cleanSourceText(sourceText, 60)
    if (cleanedText && cleanedText.length > 3) {
      if (isGenericPlatformTitle(doc.title || '', siteName) || isGenericPlatformTitle(pageTitle, siteName)) {
        if (siteName === 'Weibo' || siteName?.toLowerCase().includes('weibo')) {
          return `微博：${extractFirstTopicOrSentence(cleanedText, 60)}`
        }
        return cleanedText
      }
    }
  ...
  if (cleaned && cleaned.length > 0 && !isGenericPlatformTitle(cleaned, siteName)) {
    return cleanSourceText(cleaned, 200) || cleaned.trim()
  }
...
export function extractSourceMedia(
  ...
  if (sourceContainer) {
    try {
      ...
      for (const img of images) {
        ...
        if (isWeiboBadgeOrEmojiImage(htmlImg)) continue
        const src = htmlImg.src || htmlImg.getAttribute('src') || ''
        if (!isSafeImageUrl(src)) continue
        media.push({ type: 'image', url: src, alt: htmlImg.alt?.trim() || undefined })
  ...
  if (media.length < 5) {
    ...
    if (ogImage && isSafeImageUrl(ogImage)) media.push({ type: 'image', url: ogImage })
  }
  if (media.length < 5) {
    const twitterImage = getMetaContent(doc, 'twitter:image')
    if (twitterImage && isSafeImageUrl(twitterImage)) media.push({ type: 'image', url: twitterImage })
  }
...
function findAuthorInElement(container: Element): string | undefined {
  ...
  try {
    ...
    for (const link of links) {
      ...
      const isProfileLink = COMMENT_LINK_KEYWORDS.some((kw) => href.toLowerCase().includes(kw))
      if (isProfileLink) {
        const text = link.textContent?.trim()
        if (text && looksLikeUsername(text) && text.length < 50) return text
      }
    }
  } catch { /* query error */ }
  return undefined
...
export function resolveCommentAuthor(doc: Document, selectionRoot: Element | null): string | undefined {
...
export function buildCommentClipContext(input: CommentContextInput): CommentClipContext {
  ...
  const sourceTitle = extractSourceTitle(input.document, selectionRoot, input.siteProfileMatch, input.pageTitle)
  const sourceExcerpt = extractSourceExcerpt(input.document, selectionRoot, input.siteProfileMatch)
  const sourceMedia = extractSourceMedia(input.document, selectionRoot, input.siteProfileMatch)
  ...
  if (isGenericPlatformTitle(sourceTitle, siteName) || (sourceTitle === input.pageTitle && isGenericPlatformTitle(input.pageTitle, siteName))) {
