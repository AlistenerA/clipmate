# STORE_LISTING_DRAFT.md — ClipMate v0.3 商店文案草稿

> 上架 Edge Add-ons / Chrome Web Store 时填写。v0.3 发布候选草稿。
...
一键保存网页到 Notion。支持全文/选区剪藏、多目标管理、Markdown 多种格式输出、本地历史、内容保真增强。

---

## 简短描述（英文备选，≤132 字符）

Save web pages to Notion in one click. Full-page & selection clipping, multiple targets, Markdown profiles, local history, content fidelity enhancements.
...
ClipMate 是一款轻量级浏览器扩展，帮你把任何网页内容快速保存到 Notion，或一键复制为 Markdown。基于 Mozilla Readability 引擎自动提取正文，v0.3 新增内容保真增强，大幅提升 Markdown 和 Notion 的输出质量。

### 核心功能

- **智能剪藏**：基于 Mozilla Readability 引擎，自动提取网页正文，去除广告和侧边栏。支持全文和选区两种模式。v0.3 新增 Article Boundary Guard 预清理噪音和尾部截断。
- **Markdown Target Profiles**：支持 Notion / Obsidian / Typora / Generic Markdown 四种输出目标。Obsidian 自动含 YAML frontmatter，Typora 使用标准链接格式。一键复制完整 Markdown。
- **内容保真增强**：LaTeX 数学公式文本保留、代码块噪音清理（复制按钮/行号/语言标签）、图片/链接/表格规范化、Safe Markdown Preview（安全渲染，不执行脚本）。
...
ClipMate 专为解决这些痛点而生。v0.3 在此基础上增加了 Markdown 格式差异化输出和内容保真增强。
...
- 正文保存到 Notion 为纯文本段落，不保留 Markdown 行内格式
...
**v0.3 未新增权限**。内容保真增强功能均在现有权限内实现，不接入 AI API，不调用第三方服务。
...
1. **Popup 主界面**：展示全文模式下提取的正文、标签、备注、Markdown Target 选择器、操作按钮
2. **Options 设置页 - 多目标管理**：展示 Notion Target 列表、添加/编辑/删除/设默认操作
3. **剪藏历史 UI**：展示历史列表（含 favicon、搜索、状态标签）
4. **Notion 保存结果**：展示内容成功保存到 Notion 页面后的效果
5. **复制 Markdown 结果**：展示粘贴到 Obsidian/Typora 后的完整 Markdown 结构（含 YAML frontmatter）
