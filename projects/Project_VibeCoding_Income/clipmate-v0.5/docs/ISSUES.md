...
- 增强 Turndown `img` rule 噪声过滤、去重、alt 兜底、相对 URL 解析
- 新增 `injectMissingImages()` 在 Fullpage 末尾补充遗漏图片
...
| IS26 | base64/blob 图片 URL 处理策略未定 | minor | ✅ Session 1+2 处理：Turndown img rule 和 extractArticleImages 均过滤 |
