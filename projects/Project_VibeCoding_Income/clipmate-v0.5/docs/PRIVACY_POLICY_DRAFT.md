# PRIVACY_POLICY_DRAFT.md — ClipMate v0.5 隐私政策草稿

> v0.5 发布候选草稿。上架 Edge Add-ons / Chrome Web Store 时必须提供公开可访问的隐私政策 URL。建议将此内容托管到 GitHub Pages。
...
- **Local Clipping History**: Stored locally in your browser. Each history entry includes: page title, URL, tags, note preview, content preview, full Markdown, word count, target name snapshot, save status (saved/failed/unsaved), error codes, site name, favicon URL, theme color, and lightweight image metadata (imageCount / firstImageUrl / skippedImageCount). Full image URL lists are NOT stored. This data is used only for the history UI within the extension and is never sent to any server.
...
- **Article image URLs** (as external image blocks in Notion, NOT downloaded/uploaded/cached by ClipMate)
...
- **We do not download, upload, or cache** article images — v0.5 uses external image URLs only.
- **We do not store** full image URL lists in clipboard history — only lightweight metadata (imageCount / firstImageUrl / skippedImageCount).
...
- ClipMate does NOT output Token, full body text, full notes, complete Markdown, or full image URL lists to the browser console.
...
- **本地剪藏历史**：每条历史包含：页面标题、URL、标签、备注摘要、正文摘要、完整 Markdown、字数、目标名称快照、保存状态（已保存/失败/未保存）、错误码、站点名称、favicon URL、主题色、轻量图片元数据（imageCount / firstImageUrl / skippedImageCount）。**不保存完整图片 URL 列表**。仅用于扩展内的历史 UI，不发送到任何服务器。
...
- 您在扩展中输入的备注
- 网页正文提取结果
- **文章图片 URL**（以 external image block 方式嵌入 Notion，ClipMate 不下载/上传/缓存图片二进制）
...
- **不做**云同步
- **不调用**第三方 favicon API
- **不下载/上传/缓存**文章图片 — v0.5 仅使用 external image URL
- **不保存**完整图片 URL 列表到剪藏历史 — 仅记录轻量元数据（imageCount / firstImageUrl / skippedImageCount）
