...
### D-077：通用规则不覆盖专用 Prompt，但安全底线不可覆盖

- **原因**：保证后续每轮任务可灵活调整（功能范围、实现顺序等以专用 Prompt 为准），同时保持隐私、安全、提交边界稳定（Token 不泄露、git add . 禁用、不 push 等）。
- **影响**：WORKFLOW_RULES.md 中明确优先级分层。安全底线在任何情况下不得被覆盖。
- **可反转性**：不可逆转（安全底线为永久规则）。

### D-076：新增 V0.3_HANDOFF.md 作为 v0.3 启动前交接文档

- **原因**：v0.3 可能涉及 AI、多平台、OCR、付费等复杂方向，必须先评估需求和影响，不直接实现。交接文档提供 v0.3 候选方向评估、启动前置条件、禁止事项和审查节点，防止 v0.3 在不成熟时仓促启动。
- **影响**：v0.3 启动前必须读取 V0.3_HANDOFF.md。v0.3 Session 0 只做需求评估不写代码。
- **可反转性**：高。文档可随时调整。

### D-075：新增 WORKFLOW_RULES.md 作为跨版本通用工作流规则

- **原因**：降低后续 v0.2.x / v0.3 / v0.4 多轮 agent 开发中的上下文丢失和跑偏风险。提供统一的开发、审查、提交规范，避免每次版本迭代重新建立规则。
- **影响**：后续所有版本开发必须遵循 WORKFLOW_RULES.md 中的安全底线和流程规则。专属 Session Prompt 可覆盖功能范围但不可覆盖安全底线。
- **可反转性**：中。规则可迭代更新，但安全底线不应回退。
