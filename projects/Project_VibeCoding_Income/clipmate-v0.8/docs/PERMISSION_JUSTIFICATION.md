# PERMISSION_JUSTIFICATION.md — ClipMate v0.8 权限使用说明

> 上架 Edge Add-ons / Chrome Web Store 时提交的权限使用说明。
> 本说明对应 `manifest.config.ts` 中的实际权限配置。**v0.8 无权限变更。**
...
>
> **v0.8 权限变更**：无。Asset Picker 使用现有 content script 与消息通道；不会下载/上传图片、读取 cookie 或访问第三方私有 API。
...
- 默认标签
- 剪藏草稿（防止意外关闭 Popup 丢失编辑内容；可包含用户手选的外部图片 URL、题注和顺序）
- 本地剪藏历史（标题、URL、标签、备注摘要、正文摘要、完整 Markdown、字数、目标名称快照、保存状态、错误码、站点名称、favicon URL、主题色、轻量图片元数据 imageCount/firstImageUrl/skippedImageCount；图片 URL 只在其已属于保存 Markdown 时随 Markdown 存在）
...
**调用的 API**：
- `PATCH /v1/blocks/{page_id}/children` — 将用户剪藏的内容（含 external image block）追加到指定 Notion 页面
...
- 不会在后台自动发送请求
- 仅使用 HTTPS 加密通信
- 不调用 Notion File Upload API（上传图片到 Notion 托管）
...
**Content Script 功能（v0.5 扩展）**：
- 提取当前页面正文（通过 Mozilla Readability 算法）
- 提取用户选中文字（选区模式）
- 解析页面元数据（标题、描述、网站图标 URL、主题色）
- HTML 转 Markdown（含 LaTeX 公式保留、Code Block Cleaner、Image/Link/Table 规范化和 Article Boundary Guard）
- Fallback 提取（Readability 失败时降级为内容容器提取）
- Markdown Target Profiles（Notion / Obsidian / Typora / Generic Markdown）
- **Article Image Extraction（v0.5 新增）**：从 DOM 提取图片候选，智能过滤 avatar/icon/logo/tracking pixel 等噪声
- **Markdown Image Preservation（v0.5 新增）**：图片语法保留、去重、相对 URL 解析
- **Image Metadata Collection（v0.5 新增）**：收集 imageCount/firstImageUrl/skippedImageCount，不下载图片二进制
- **Asset Picker（v0.8 新增）**：用户主动启动后，临时显示 Shadow DOM 覆盖层并读取可见图片 URL；完成、取消或页面离开时清理
...
- ✅ **不会永久修改**页面内容或候选图片样式；仅在用户主动选择期间挂载隔离的 Shadow DOM 覆盖层，退出时完整清理
- ✅ **不会向第三方发送**页面数据
- ✅ **不会自行发起**网络请求（所有 API 调用由 Background Service Worker 完成）
- ✅ Content Script 中的 `console.log` 仅输出脱敏后的状态信息，不输出正文全文或完整图片 URL 列表
- ✅ **favicon 提取不发起网络请求**：从页面 DOM 读取 URL 字符串
- ✅ **图片提取不发起网络请求**：从 DOM 读取 img 元素属性，不下载/上传/缓存图片二进制
- ✅ **不使用 FileReader/canvas/toDataURL/createObjectURL**
...
| 权限 | v0.1 | v0.2 | v0.3 | v0.4 | v0.5-v0.8 | 说明 |
|------|:---:|:---:|:---:|:---:|:---:|------|
| `activeTab` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
| `storage` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
| `host_permissions: api.notion.com` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
| `content_scripts all_urls` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变（v0.5 额外提取图片 URL 等，但仍从 DOM 读取）|

**v0.8 新增功能不依赖任何新增权限**。Asset Picker 只读取当前页面可见图片属性并显示临时覆盖层；只允许 HTTP(S) URL，不下载/上传图片、不读取 cookie。不调用 Notion File Upload API，不新增 FileReader/canvas/toDataURL/createObjectURL。
...
- 用户剪藏内容（含文章图片 URL）仅通过用户手动操作发送到用户自己的 Notion 工作区
...
> ClipMate v0.5 的 `<all_urls>` content_scripts 是为了支持用户在任意网站进行网页剪藏。Content Script 仅在用户主动点击扩展图标时才执行正文提取，不会在后台持续运行。提取的内容由 Background Service Worker 通过 HTTPS 直接发送到用户自己的 Notion API，不经过任何第三方服务器。v0.5 新增的文章图片保存功能（Article Image Extraction、Markdown Image Preservation、Notion External Image Blocks、Lightweight Image Metadata）全部基于现有的 `storage`、`activeTab` 和 `host_permissions` 权限实现，使用 external image URL 策略，不下载/上传/缓存图片二进制，不调用 Notion File Upload API，未新增任何 manifest 权限，不接入 AI 或外部 API。
