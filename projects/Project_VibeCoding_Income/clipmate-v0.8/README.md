...
v0.8 定位为 **Asset Picker（用户可控图片版）**：在自动提取基础上允许用户从当前页面手动补充、移除和排序图片，同时继承 v0.7 Tutorial Mode。
...
3. 开启「开发人员模式」
4. 点击「加载解压缩的扩展」，选择 `clipmate-v0.8/dist/` 目录
...
生成 `clipmate-v0.8.zip`，仅包含 dist/ 构建产物，不包含源码、测试、文档、node_modules。
