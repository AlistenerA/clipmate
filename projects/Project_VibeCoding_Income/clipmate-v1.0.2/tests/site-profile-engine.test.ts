...
describe('matchSiteProfile', () => {
  ...
  describe('social and community profiles', () => {
    ...
    it('matches tieba.baidu.com/p/xxx to tieba-community instead of Baidu Search', () => {
      const result = matchSiteProfile({
        url: 'https://tieba.baidu.com/p/123456',
        pageType: 'forum-or-comment',
      })
      expect(result).not.toBeNull()
      expect(result!.profile.id).toBe('tieba-community')
    })
...
describe('seed profiles structural QA', () => {
  ...
  it('counts exactly 25 seed profiles', () => {
    expect(SEED_PROFILES.length).toBe(25)
  })
