...
## v1.0.2 发布收尾（2026-06-21）

- 获准环境完成 staging build：186 modules，manifest 1.0.2，License host 仅 `cydl.site`。
- production/staging 权限与 host 隔离、production 归档完整性和敏感信息扫描通过。
- 按产品负责人要求缩减非必要重复浏览器环节；真实 built-extension/Notion 保存明确保留为后续抽查风险。

## v1.0.2 社区站点剪藏初始化（2026-06-21）

- 新建独立 `clipmate-v1.0.2/`，从已验证 v1.0.1 工作区复制，排除构建产物、依赖、ZIP 和用户图像资源。
- package/manifest/lockfile 更新到 `1.0.2`，发布包名更新为 `clipmate-v1.0.2.zip`。
- 确立小红书与百度贴吧 DOM-only 专用提取范围：主体图文、最多 50 条评论/回复、无关链接过滤。
- 新增 `src/content/socialPost/` 纯 DOM 提取模块，支持两站路由检测、正文/图片/评论提取、虚拟列表有界滚动与 50 条硬上限。
- 小红书轮播图片按 swiper 索引排序去重；贴吧首楼、楼层和楼中楼回复映射为统一语义输出。
- `EXTRACT_CURRENT_PAGE` 优先使用站点专用结果，其他页面保持原有通用全文/选区/教程路径。
- 生成的 Markdown 仅保留规范化原帖来源链接，站内无关链接转为纯文本；HTML 结果不含锚点。
- 新增 6 项专用提取测试并扩展站点档案回归；当前全量 70 files / 2067 tests 通过。
