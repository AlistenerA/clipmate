# DECISIONS.md — ClipMate v0.9 技术决策记录

## v1.0.2 发布收尾决策（2026-06-21）

- 以已完成的登录态 DOM 观察、专用 fixture、70 files / 2067 tests、双模式构建、权限隔离和归档校验作为本版必要门禁。
- 按产品负责人明确指令，不为 v1.0.2 重新搭建登录态扩展自动化环境；真实 built-extension 与 Notion 保存保留为后续抽查，文档不得宣称已执行。

## v1.0.2 社区站点剪藏决策（2026-06-21）

- 采用站点专用 DOM 提取器优先、现有通用全文提取兜底，不新增 manifest 权限或远程服务。
- 不调用小红书/贴吧私有 API，不读取 Cookie，不绕过登录；Playwright 仅在有头窗口中由用户自行登录。
- 评论/回复按 DOM 顺序最多保留 50 条；嵌套回复计入同一总上限，避免输出和 Notion block 无界增长。
- 输出只保留来源、作者/楼层、正文、时间、主题标签与正文图片；移除导航、推荐、关注和统计跳转链接。

## v1.0.1 临时购买入口与 Windows GUI 决策（2026-06-21）

- 服务器网站暂停开发期间，插件购买入口直接跳转爱发电方案页，避免根域 404 和安全软件风险提示。
- 升级提示作为共享组件，所有入口明确告知服务器网站仍在开发中，不伪装为已上线官网。
- Python 管理工具只依赖标准库与 Windows 自带 OpenSSH/ACL 工具；GUI 使用 Tkinter，不引入打包或前端运行时。
- GUI 与 CLI 复用同一请求验证、SSH、解析、ACL 和失败补偿逻辑，不维护第二套安全实现。
- 完整 Key 不进入 GUI、CLI stdout 或剪贴板；PowerShell `-PassThru` 仅作为既有兼容能力保留。

## v1.0.1 Windows License 管理决策（2026-06-21）

- Windows 管理端复用生产 SSH 和服务端 CLI，不新建公网管理 API，也不扩大服务器端口或插件权限。
- 备注不直接拼接进 shell；先编码为 Base64，完整远程脚本再次编码后交给固定 `/bin/bash` 执行。
- 默认不把完整 Key 写入终端；Key 仅进入当前用户与 SYSTEM 独占的本地文件，自动化消费必须显式使用 `-PassThru`。
- SSH 使用 `BatchMode=yes`，避免脚本停在密码提示；服务器返回必须且只能包含一个合法 Key，并与请求套餐和设备数一致。
- 远程创建与 Windows 安全落盘按补偿事务处理；落盘前任一步失败都自动调用正式 CLI 吊销新 Key。

## v1.0.1 决策（2026-06-21）

- v1.0.1 从冻结 v0.9.3 复制到独立目录和分支，历史目录不修改。
- 现有能力保持免费；Pro 初始功能键只作为未来入口，不在本版实现批量/AI。
- License 网络只由 Background 发起，base URL 与 host permission 从同一构建模式生成。
- Token 存在 `chrome.storage.local` 且仅 24 小时有效；不存完整 License Key。
- 暂时网络错误保留七天宽限，明确吊销/过期/设备无效立即清除。
- Onboarding 只由 fresh install 写 pending；update 不写，完成后返回主 Popup。
- Onboarding 用 Rollup 多入口打包，不额外加入 manifest web accessible resources。
- 正式 API 固定为 `https://license.cydl.site/api`；端点按 Vite mode 由可追踪配置生成，不依赖被忽略的 `.env.*` 文件。
- 浏览器 UI 不直接持有跨域网络能力，License 请求统一经 Background Service Worker 发送。
- 开发工具链公告不通过盲目跨大版本升级混入 v1.0.1；production audit 必须为 0，Vite/Vitest 迁移单独验证 CRXJS 兼容性。
...
- 页面感知只传递分类结果与布尔信号；原始 detector signals、DOM、正文、选区文本和 URL 不进入新字段。
- 推荐只改变默认排序和首次高置信度选择，不删除模式；“更多模式”永久提供全部既有能力。
- 只有无选区的高置信度视频/AI 页允许首次自动采用；草稿恢复和用户操作具有更高优先级。
- 文章代码页只推荐教程模式，不自动切换，避免单个代码片段改变普通文章的用户预期。
- 页面感知字段为可选 additive data，不提高 storage schema 版本，不迁移 v0.8 草稿。

---

## v0.8.5 决策（2026-06-19）

- 悬停状态只绑定当前候选；滚动后不自动跳到指针下的新图片，避免用户未移动鼠标却高亮另一资源。
- History additive fields 保持可选，旧记录继续按 Notion 保存记录解释，无需 storage schema 迁移。
- 复制记录使用 `unsaved` 兼容既有状态类型，但 UI 以 `action=markdown-copy` 显示“已复制”并禁止重试。
- Notion 链接仅由 32 位十六进制 page ID 本地构造，渲染前再次限定 HTTPS + `notion.so` 主机。
...
- 当前草稿仍可通过既有 `LAST_CLIP` 恢复手选图片，这是草稿持久化而不是站点资源缓存。
- 覆盖层使用 Shadow DOM 与独立定位框，不修改候选图片的 class/style，避免污染宿主页面。
- active picker 会话拦截页面点击以避免误导航；工具条事件通过 composed path 明确放行。
- 候选复用现有 `extractArticleImages` 和 Asset Pipeline，不维护第二套 lazy/srcset/噪声 URL 规则。
- 只允许 HTTP(S) 图片进入草稿；data、blob、javascript、隐藏元素、追踪像素与噪声图片在页面侧和草稿侧双重过滤。
- 选择上限固定为 20，不新增权限、网络请求、下载、上传、OCR 或第三方 API。
- 不重建整个教程 ClipDocument；Notion adapter 在结构化 blocks 后安全追加缺失的手选图片，并按 URL 去重。
- V0.8 只在 `codex/clipmate-v0.8-asset-picker` 与 `clipmate-v0.8/` 中提交，V0.7 分支和目录保持冻结。
...
### D-v0.7-006：站点代码容器用窄规则恢复，图片预览只加载安全 HTTP(S) URL

- **原因**：真实 Runoob 页面用 `div.example_code` 而不是 `pre/code`，通用 Turndown 会压平代码；Popup 原本把图片显示为文字，无法发现图片缺失或位置错误。
- **影响**：只识别少量明确代码容器 class，并用内容特征保守推断语言；Popup 图片预览仅允许 HTTP(S)，使用 `no-referrer`、懒加载和失败降级。不会执行页面脚本、加载 data/blob URL 或新增权限。
- **可反转性**：中。代码容器 allowlist 和语言启发式可按 fixture 继续扩展；图片预览可后续增加用户开关。

### D-v0.7-007：Notion 头部使用 body callout，不伪造数据库属性

- **原因**：ClipMate 当前语义是向用户选择的页面 append blocks，并不知道目标是否为数据库页面及其 property schema。直接模仿参考截图写数据库属性会要求额外 API、目标类型探测和属性映射。
- **影响**：标题后增加一个 `🔖` callout，合并来源、站点、作者、发布日期、模式、剪藏时间和标签。继续使用公开 append blocks API，不新增权限或私有接口。
- **可反转性**：高。v0.8 SaveTargetAdapter 可针对 database/data source 目标提供显式 property mapping。

### D-v0.7-008：错误反馈只透传短码、状态与批次，不透传 Notion message

- **原因**：Notion `message` 可能包含请求路径或用户内容片段；用户需要的是可定位的简短原因，而不是原始响应正文。
- **影响**：客户端只 allowlist API `code`，映射为内部错误码，并附带 HTTP status 与 1-based batch。Popup 和 History 都不保存响应 message、Token、Page ID 或 block 内容。
- **可反转性**：低。敏感响应最小化是长期安全边界。
