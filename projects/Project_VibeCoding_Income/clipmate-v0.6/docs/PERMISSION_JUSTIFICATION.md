# PERMISSION_JUSTIFICATION.md — ClipMate v0.5 权限使用说明

> 上架 Edge Add-ons / Chrome Web Store 时提交的权限使用说明。
> 本说明对应 `manifest.config.ts` 中的实际权限配置。**v0.5 无权限变更。**
...
>
> **v0.5 权限变更**：无。v0.5 所有新增功能（Article Image Extraction、Markdown Image Preservation、Notion External Image Blocks、Lightweight Image Metadata）均在现有权限范围内实现，未新增任何 manifest 权限。
...
- 本地剪藏历史（标题、URL、标签、备注摘要、正文摘要、完整 Markdown、字数、目标名称快照、保存状态、错误码、站点名称、favicon URL、主题色、轻量图片元数据 imageCount/firstImageUrl/skippedImageCount。**不保存完整图片 URL 列表**）
...
| 权限 | v0.1 | v0.2 | v0.3 | v0.4 | v0.5 | 说明 |
|------|:---:|:---:|:---:|:---:|:---:|------|
| `activeTab` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
| `storage` | ✅ | ✅ | ✅ | ✅ | ✅ | 不变 |
...
**v0.5 新增功能不依赖任何新增权限**。Article Image Extraction、Markdown Image Preservation、Notion External Image Blocks、Lightweight Image Metadata 等功能均在现有权限范围内实现。不调用 Notion File Upload API，不下载/上传/缓存图片，不新增 FileReader/canvas/toDataURL/createObjectURL。
