# STORE_SUBMISSION.md — ClipMate v0.9 商店提交材料

> 上架 Edge Add-ons / Chrome Web Store 时填写。v0.9.0 发布候选。
...
一键保存网页内容到 Notion。按页面智能推荐全文、选区、教程或视频收藏，并支持图片点选与本地历史。

---

## 简短描述（英文备选，≤132 字符）

Save web content to Notion with page-aware modes, tutorial structure, selected images, Markdown profiles, and local history.
...
ClipMate 是一款轻量级浏览器扩展，帮你把网页内容快速保存到 Notion，或复制为 Markdown。v0.9 根据当前页面结构推荐合适模式，并继续保留 Asset Picker 与教程结构。

### 核心功能

- **智能剪藏**：基于 Mozilla Readability 引擎，支持全文、选区和教程三种模式。
- **页面感知模式 (v0.9)**：识别文章、视频、讨论、AI 对话、搜索和导航页面，显示推荐与理由，用户可展开并切换全部模式。
- **手动图片点选 (v0.8)**：在当前页选择自动提取遗漏的图片，支持预览、移除、排序和安全过滤。
- **教程结构保真 (v0.7)**：标题、代码语言、公式、表格、callout、图片题注和视频链接保存为原生 Notion blocks。
- **站点适配与场景模式 (v0.4)**：
...
- 视频仅保存链接元数据，不下载视频或抓取字幕
- 外部图片是否显示取决于源站允许 Notion 访问
- 不支持复杂合并单元格表格的完全还原
...
**v0.9 无权限变更。** 页面感知仅分析当前 DOM 的结构统计，不发起网络请求、不读取 cookie，也不上传 detector 信号。
...
- 当前草稿可保存用户手选的外部图片 URL；不保存图片二进制，不建立站点级缓存
...
- v0.9 PageAwareness 只保留页面类型、置信度、选区布尔值和固定推荐结果，不保存原始 signals、DOM 或正文
...
**v0.9.0 自动化候选**：62 files / 2028 tests、lint、build 和本地 Playwright Popup 交互均通过。Chrome/Edge unpacked 与真实 Notion 保存仍建议在首次商店提交前冒烟。
